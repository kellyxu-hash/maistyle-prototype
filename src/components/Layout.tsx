...
import { Folder, Grid3x3, Package, House } from "lucide-react";
...
export function Layout() {
  ...
  return (
    <div
      className="flex h-screen w-screen overflow-hidden pt-3 pr-3 pb-3"
      style={{ background: "linear-gradient(135deg, #DCB297 0%, #DE9BD2 50%, #7575B1 100%)" }}
    >
      {/* Sidebar */}
      <aside className="w-[92px] flex flex-col items-center flex-shrink-0">
        {/* Logo */}
        <div className="h-[70px] w-full flex items-center justify-center">
          <img src="/logo.png" alt="Maistyle" className="h-[25px] w-auto brightness-0 invert" />
        </div>

        {/* Navigation */}
        <nav className="flex-1 w-full flex flex-col gap-1 px-2 pt-[40px]">
          <button
            onClick={() => navigate("/")}
          ...
          >
            ...
            <span className={`text-[13px] tracking-[-0.15px] text-white ${isActive("/") ? "font-semibold" : ""}`}>
              Home
            </span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-2 py-3 rounded-lg transition-colors ${
              isActive("/brand") ? "bg-white/20" : "hover:bg-white/10"
            }`}
          >
            <Package className="w-5 h-5 text-white" />
            <span className={`text-[13px] tracking-[-0.15px] text-white ${isActive("/brand") ? "font-semibold" : ""}`}>
              Brand
            </span>
          </button>
          ...
          >
            ...
            <span className={`text-[13px] tracking-[-0.15px] text-white ${isActive("/projects") ? "font-semibold" : ""}`}>
              Projects
            </span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-2 py-3 rounded-lg transition-colors ${
              isActive("/exports") ? "bg-white/20" : "hover:bg-white/10"
            }`}
          >
            <Grid3x3 className="w-5 h-5 text-white" />
            <span className={`text-[13px] tracking-[-0.15px] text-white ${isActive("/exports") ? "font-semibold" : ""}`}>
              Exports
            </span>
          </button>
        </nav>
      ...
      <main className="flex-1 bg-white rounded-2xl overflow-auto shadow-xl">
        <Outlet />
      </main>
    </div>
  );
